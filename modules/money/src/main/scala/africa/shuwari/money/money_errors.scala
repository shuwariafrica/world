...
import africa.shuwari.money.conversion.ConversionQuery // For creating lightweight error objects
...
object errors:
  ...
  /** A root error for all operations in the `africa.shuwari.money` module.
    *
  ...
  /** Returned when a `String` cannot be parsed into a numeric representation.
    ...
    * [[africa.shuwari.money.currency.CurrencyValue$.fromString]].
    *
  ...
  object ConversionError:
    ...
    /** Returned if an exchange rate cannot be found for a given conversion
      * query.
      *
      * @param query The [[africa.shuwari.money.conversion.ConversionQuery]]
      *   that failed.
      */
    ...
    /** Returned if an error occurs within an
      * [[africa.shuwari.money.conversion.ExchangeRateProvider]].
      *
