...
object CountriesPopulator {
  ...
  private def generateCountriesFileSource(countries: SortedSet[ParsedCountryData], retrievedDate: String): String = {
...
import africa.shuwari.locale.errors.{DuplicateCountryData, LocaleError}

/** A sealed trait representing a country or area with its primary identifiers. */
sealed trait Country extends Product with Serializable with Matchable derives CanEqual:
  def name: String
...
final case class GenericCountry private[country] (
    name: String,
    alpha2: Alpha2Code,
    alpha3: Alpha3Code,
    m49: M49Code)
...
object Country:
  ...
  /** Creates a custom `Country` instance if the provided data is valid and does not
    ...
    * @return A `Right` containing a new [[GenericCountry]], or a `Left` with a [[africa.shuwari.locale.errors.LocaleError]].
    */
  def generic(
      name: String,
      alpha2: String,
    ...
    def unique[A](label: String, value: A)(predicate: Country => Boolean): Either[LocaleError, Unit] =
      Either.cond(!summon[Set[Country]].exists(predicate), (), DuplicateCountryData(s"$$label '$$value' is already in use."))
    for {
...
/** Provides predefined singleton instances for all countries/areas and lookup methods.
  * @note This file is automatically generated. Data sourced from UNSD, updated around: $retrievedDate.
  */
object Countries:
